...
var url = require("url"),
    ...
    address, output, pixelRatio, width, height, outputFolder;

var args = Array.prototype.slice.call(system.args, 1);
if (args.length != 5) {
    console.log('Usage: pixelratio.js URL output width height pixelRatio');
    phantom.exit();
}
console.log("XXXXX");
...
// if (fs.lstatSync(outputFolder) && !fs.lstatSync(outputFolder).isDirectory()) {
//     console.log('output must be a directors');
//     phantom.exit();
// }
//
// output = outputFolder + "/" + url.parse(address).pathname.replace(/\//gi, "_") + ".png";
//
